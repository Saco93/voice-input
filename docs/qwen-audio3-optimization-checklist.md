...
- [~] API verification and deterministic runtime implementation complete. Named preset resolution, presence-aware migration, documented `multi_threshold_mode_enabled`, optional `speech_noise_threshold`, universal finite-threshold validation for Settings JSON serialization, active-custom effective range/combination validation, request omission rules, Settings controls, diagnostics, privacy coverage, and bilingual documentation are implemented locally. Separate speech/noise threshold fields do not exist in the Audio3 documentation and remain blocked. Item 5 remains in progress pending controlled live evaluation.
- [x] Define `standard`, `low-latency-dictation`, `long-form`, and `custom` presets instead of exposing unexplained raw values by default. Standard preserves the existing request. The `400` ms low-latency and `1300` ms semantic long-form mappings are explicitly labeled as evaluation candidates, not validated recommendations.
...
- [~] Bounded parsing and aggregate diagnostics are complete and validated locally. Identity-dependent assembly remains blocked, and live observation of provider event shapes is still pending.
...
- [ ] Observe authorized live Audio3 partial/final event shapes for parser compatibility and counterexample discovery only; do not infer a stable revision identity contract from finite captures.
