...
用户还可以启用**原生最终处理**：流式识别结束后，程序会把完整录音发送给 `qwen-audio-3.0-asr-flash`。成功的原生 transcript 优先级最高。如果原生服务返回无词结果，并且没有可用的流式 transcript，该结果具有最终效力；如果已有可用的流式 transcript，程序会保留它。原生请求失败时，程序会保留可用的流式文本；仍无可用文本时，可以使用已配置的本地备用识别。原生请求最多接受 10 MiB 的原始 WAV 音频。
