...
Qwen-Audio-3 is the default ASR provider. Add the Alibaba credential, keep or disable the local fallback as needed, and enable LLM or agent context only when wanted. Credentials are encrypted through the standard per-user systemd credential store.
...
**Endpoint mode** defaults to **Regional**, with **Beijing** as the default region; **Singapore** is also selectable. Regional mode selects the fixed reviewed Streaming and Native hosts for the chosen region. **Custom** mode uses the configured Streaming and Native URLs exactly, including path, port, and query bytes.
...
Both commands require Qwen-Audio-3 to be selected in the active configuration. Remote tests send the supplied audio to the resolved Regional route or exact Custom endpoint and may incur API charges.
