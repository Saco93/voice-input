...
```mermaid
flowchart LR
    Mic[Microphone<br/>16 kHz PCM] --> A3[Qwen-Audio-3 Streaming]
    A3 --> Final[Optional Audio3 Native final pass]
    Final --> LLM[LLM refinement<br/>15 s default budget]
    A3 -. streaming result .-> LLM
    LLM --> Out[Wayland / XWayland output]
    A3 -. live transcript .-> HUD[Quickshell HUD]
    Local[Local CLI fallback] -. remote recovery .-> LLM
...
2. Qwen-Audio-3 Streaming sends partial text to the HUD through a bounded, nonblocking queue and fair bidirectional WebSocket processing. After one recoverable transport interruption before `finish-task`, the worker creates one replacement task and replays every retained raw PCM packet from the beginning while recording continues.
3. On toggle-off, the streaming task produces the primary final transcript. The optional Qwen-Audio-3 Native final pass can recognize the complete recording again in Adaptive or Always mode. If streaming recovery fails, remote delivery falls behind, or the selected Native pass fails without a usable streaming result, Voice Input uses the configured local fallback.
...
No-speech sessions return to idle once streaming, Native, or local ASR confirms that no transcript exists. Audio capture, ASR, HUD rendering, persistence, and output are isolated so a slow visual or clipboard client cannot block recognition.
...
Qwen-Audio-3 is the default ASR provider. For a new setup, keep **Workspace** endpoint mode, select **Beijing** or **Singapore**, and copy your real **Workspace ID** from the Alibaba console; do not paste a full URL. Add an Alibaba API key authorized for that region, workspace, and the selected models. The Rust backend derives the Streaming, Native, and Alibaba Qwen refinement API addresses, so no URL editing is needed. Keep or disable the local fallback as needed, and enable LLM or agent context only when wanted. Credentials remain encrypted through the standard per-user systemd credential store.
...
Settings → **Refinement** offers **GPT OSS 120B (OpenRouter)**, **Qwen3.8 27B (Alibaba Beijing)**, and **Custom** model choices. The Alibaba Qwen preset sets `llm.endpoint_mode = "alibaba-workspace"` and uses Audio3's region, Workspace ID, and encrypted Alibaba credential. The Rust backend derives its OpenAI-compatible HTTP base without modifying the dormant custom `api_base_url`. OpenRouter uses the independent `custom` endpoint mode. Selecting a Custom model preserves the current endpoint mode; choose Custom endpoint mode to edit an independent URL. LLM remains disabled with `endpoint_mode = "custom"` in the public example. The Reasoning level selector offers Off/Low/Medium/XHigh for Qwen and Low/Medium/High for GPT OSS; GPT OSS requires reasoning. Existing configurations retain Qwen Off and GPT OSS Medium. Unknown custom models use their provider default. Custom retains editable model, endpoint, and credential selection. **Test LLM** tests the draft without saving; **Save** applies the selection. Existing configurations retain their current model, endpoint, and credential until a different preset or route is explicitly selected.
...
Qwen-Audio-3 is the default remote provider and the primary recognition path. Local CLI remains available as an explicit provider and as the optional fallback after remote failure. Configure the encrypted Alibaba credential and Audio3 options in Settings. See the Wiki [Architecture](https://github.com/Saco93/voice-input/wiki/Architecture) and [Configuration](https://github.com/Saco93/voice-input/wiki/Configuration) pages for the detailed flow and option reference.

**Endpoint mode** defaults to **Workspace** (`workspace`) for new configurations, with **Beijing** (`beijing`) as the default region and an empty `workspace_id`. **Singapore** (`singapore`) is also selectable. These are the regions Voice Input offers for current Audio3 support, not a claim that all Alibaba Model Studio services have only two regions. Supply the console Workspace ID; the Rust backend derives the workspace-domain Streaming and Native URLs from that ID and region. **Regional** (`regional`) retains the legacy shared-domain pair for the selected region. **Custom** (`custom`) uses the configured Streaming and Native URLs exactly, including path, port, and query bytes. Workspace and Regional modes preserve dormant `endpoint` and `native_endpoint` strings.

A first-run template with a blank Workspace ID can load so Settings can open. Saving an active workspace route requires a valid ID; Settings rejects invalid IDs and full URLs entered as IDs. Runtime sends no request through a workspace route without a valid ID. The same requirement applies to Alibaba workspace refinement.

Existing file configurations keep their former routing until the user explicitly selects Workspace mode and supplies an ID. Migration is presence-aware. A configuration without `endpoint_mode` migrates to Regional Beijing only when both old URLs exactly equal the canonical Beijing pair, or to Regional Singapore only when both exactly equal the canonical Singapore pair. Mixed pairs, noncanonical hosts, loopback endpoints, proxies, and any custom path, port, or query migrate to Custom with both URL strings preserved byte-for-byte. An explicit endpoint mode takes precedence, and an explicit region takes precedence with Beijing as its missing-field default. Dormant raw URLs remain preserved.

A legacy `alibaba-qwen-realtime` selection migrates to Qwen-Audio-3. Canonical Beijing and Singapore realtime endpoints retain their region. A custom realtime route remains on the same origin in Custom mode while its streaming path is changed from `/realtime` to `/inference` and its Native path is derived on that origin; verify proxy-specific paths in Settings. A plaintext Alibaba key is never discarded by a generic config rewrite: open and save Settings once to move it into the encrypted credential store.

The Alibaba API key must match the selected region and workspace and have grants for the selected models. Changing the region or workspace may require replacing the encrypted Alibaba credential. Voice Input never probes another region and never migrates a key automatically. Singapore availability does not establish feature parity: each model, control combination, and language/vocabulary scenario still requires authorized live validation. No live API test was run for the workspace-routing change.

The streaming model supplies realtime text. When session terminology is enabled and dictation starts in a validated Pi or Codex session, its `run-task` also receives at most 400 characters of locally redacted, low-frequency-first Session Context terminology; no `continue-task` event is used. On one recoverable transport interruption before `finish-task`, Voice Input creates a new Audio3 task, discards the old task's transcript, and replays retained PCM from the beginning at 4× realtime while recording continues. Retention is prefix-complete and limited by the configured recording duration, 300 seconds, and 10 MiB of PCM; exceeding the limit disables reconnect instead of retaining or replaying an incomplete prefix. A second interruption or a post-finish interruption uses the existing Native/local complete-audio recovery. **Language hints** and **streaming heartbeat** are independent opt-in settings and are disabled by default. Enabling language hints sends the existing language selection to Audio3: English uses `en`; Simplified and Traditional Chinese use `zh,en`; Japanese uses `ja,en`; and Korean uses `ko,en`. The extra English hint retains mixed-English recognition for Chinese, Japanese, and Korean; leaving the switch disabled preserves the provider's automatic detection. Enabling streaming heartbeat keeps long silent push-to-talk sessions alive while correctly formatted audio frames continue.
...
Both commands require Qwen-Audio-3 to be selected in the active configuration. Remote tests send the supplied audio to the resolved Workspace or Regional route, or the exact Custom endpoint, and may incur API charges.

## Highlights

- Qwen-Audio-3 streaming dictation with mixed-language hints and live partial transcripts
